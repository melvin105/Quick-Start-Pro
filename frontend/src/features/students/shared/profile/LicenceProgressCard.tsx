...
import { DEFAULT_LICENCE_PROGRESS, buildLicenceSteps, type LicenceStepStatus } from '../licence'
import { studentLicencePath } from '../utils'
...
export default function LicenceProgressCard({ student }: { student: Student }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl p-5 flex flex-col gap-3">
      ...
      <Link
        to={studentLicencePath(student.id)}
        className="mt-1 self-start flex items-center gap-1 text-[12px] font-medium text-brand-600 hover:text-brand-700"
      >
        Update Licence <ChevronDown size={13} />
      </Link>
    </div>
  )
}
