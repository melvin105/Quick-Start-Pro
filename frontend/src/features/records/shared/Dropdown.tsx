  ...
  return (
    <div className="relative w-full">
      <button
        type="button"
        onClick={() => setOpen((p) => !p)}
        className="w-full flex items-center justify-between gap-2 px-3 py-2 text-[13px] text-gray-900 bg-white border border-gray-200 rounded-lg hover:border-gray-300 transition-colors"
      >
        {selected?.label}
      ...
      {open && (
        <>
          ...
          <ul className="absolute left-0 z-20 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-card overflow-hidden max-h-60 overflow-y-auto">
            {options.map((opt) => (
              <li key={opt.value}>
                <button
                  type="button"
